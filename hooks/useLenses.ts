  ...
  const searchLenses = useCallback(async (filters: FilterState) => {
    ...
    try {
      ...
      if (filters.sph !== "" && isNaN(Number(filters.sph))) {
        setError("Please enter a valid SPH value");
        return;
      }
      if (filters.cyl !== "" && isNaN(Number(filters.cyl))) {
        setError("Please enter a valid CYL value");
        return;
      }

      let searchSph : any = filters.sph;
      let searchCyl : any = filters.cyl;
      ...
      if (searchSph !== "" && searchCyl !== "") {
        ...
        searchSph =  parseFloat(transposed.sph as any);
        searchCyl = parseFloat(transposed.cyl as any);
      }
